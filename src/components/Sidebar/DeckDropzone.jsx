  ...
  return (
    <div className="deck-dropzone" onDrop={handleDrop} onDragOver={e => e.preventDefault()}>
      ...
      <div className="deck-cards-grid">
        {deck.map(({ card, count }) => (
          <div key={card.id} className="deck-card-wrapper" onClick={() => onRemoveOne(card)} title="Clicca per rimuovere 1 copia">
            <img src={card.immagine} alt={card.nome} className="deck-card-img" />
            <span className={`card-count-badge ${count === 3 ? 'max-copies' : ''}`}>{count}</span>
          </div>
        ))}
      </div>
